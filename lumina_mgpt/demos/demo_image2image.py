...
def model_worker(
    ...
    while True:
        ...
        try:
            prompt, input_image, task, seed, gen_t, cfg, image_top_k = request_queue.get()

            random_seed(seed=seed)

            if task == "Semantic Segmentation":
                prompt = "Segmentic segmentation."
            elif task == "Depth Estimation":
                prompt = "Depth estimation."
            elif task == "Surface Norm Estimation":
                prompt = "Surface normal estimation."
            elif task == "Human Pose Estimation":
                prompt = "Human pose estimation."
            elif task == "Detection":
                prompt = "Detect: " + prompt
            elif task == "Editing":
                pass
            elif task == "Condition to Image":
                prompt = f"Generate an image according to the provided image, and according to the following caption:\n{prompt}"  # noqa
...
def gradio_worker(
    ...
    def updateUIForTask(task):
        if task == "Detection":
            return gr.update(label="Object to Detect", visible=True)
        elif task == "Editing":
            return gr.update(label="Editing Prompt", visible=True)
        elif task == "Condition to Image":
            return gr.update(label="Image Prompt", visible=True)
        else:
            return gr.update(visible=False, value="")

    def stream_model_output(prompt, input_image, task, seed, gen_t, cfg, image_top_k):
        ...
        for queue in request_queues:
            queue.put((prompt, input_image, task, seed, gen_t, cfg, image_top_k))
        while True:
            content_piece = response_queue.get()
    ...
    with gr.Blocks(css="#image_input {height: 100% !important}") as demo:
        gr.Markdown("# Lumina-mGPT Image2Image Demo\n")
        with gr.Row() as r:
            with gr.Column(scale=1):
                with gr.Row():
                    input_image = gr.Image(type="pil", label="Input Image", interactive=True, elem_id="image_input")
                with gr.Row():
                    l_tasks = [
                        "Semantic Segmentation",
                        "Depth Estimation",
                        "Surface Norm Estimation",
                        "Detection",
                        "Human Pose Estimation",
                        "Editing",
                        "Condition to Image",
                    ]
                    task = gr.Dropdown(value=f"Semantic Segmentation", choices=l_tasks, label="tasks")
                with gr.Row():
        ...
        submit_button.click(check_input_sanity, [prompt], []).success(
            stream_model_output, [prompt, input_image, task, seed, gen_t, cfg, image_top_k], [output_img, real_prompt]
        ).success(lambda: gr.update(visible=True), [], [real_prompt])
    barrier.wait()
